...
enum Punch {
    ...
    /// Is this rendezvous record actually our paired device?
    ///
    /// It may not be. The record name is fixed per role (`peer-phone`), so a
    /// *second* device of that role signed into the same Apple ID — a
    /// Simulator during development, a family iPad, a replaced handset —
    /// overwrites it wholesale, identity key included. Pairing survives that,
    /// because the channel key each side stores is untouched; the rendezvous
    /// does not. Punching at whatever addresses happen to be in the record
    /// means firing packets at a stranger forever while the real peer waits.
    ///
    /// So the identity in the record is checked against the device we paired
    /// with, and a mismatch is refused loudly rather than acted on.
    static func isPairedPeer(_ record: PeerRecord) -> Bool {
        guard let paired = PairingStore.load() else { return false }
        guard paired.pubKeyAgreement == record.pubKeyAgreement else {
            logger.error("""
            rendezvous record belongs to '\(record.deviceName, privacy: .public)', \
            not the paired '\(paired.name, privacy: .public)' — ignoring its addresses. \
...
final class PunchListener {
    /// Called on the main queue with each accepted stream.
    var onStream: ((PunchTransport) -> Void)?
    ...
    private var peerCandidates: [PeerEndpoint] = []
    ...
    /// While the phone's connect request is recent, punch hard.
    private var urgentUntil = Date.distantPast
    ...
    func start() {
        Punch.queue.async { [self] in
            ...
            guard let channelKey = try? PairingStore.channelKey() else { return }
            do {
                ...
                timer = t
                Punch.logger.notice(
                    "punch listener up on local port \(socket.localPort(), privacy: .public)"
                )
            } catch {
                Punch.logger.error(
    ...
    private func tick() {
        ...
        if now >= nextPingAt, !peerCandidates.isEmpty {
            let urgent = now < urgentUntil
    ...
    private func sendProbes() {
        guard let socket, let key else { return }
        let ping = PunchPacket(kind: .ping).encoded(key: key)
        for candidate in peerCandidates {
    ...
    private func pollPeer() {
        Task { [cloud] in
            let peer: PeerRecord?
            do {
                peer = try await cloud.fetchPeer(of: .current)
            } catch {
                Punch.logger.error(
            ...
            guard let peer else {
                Punch.logger.notice("no peer record yet — phone hasn't published")
                return
            }
            guard Punch.isPairedPeer(peer) else {
                Punch.queue.async { [weak self] in self?.peerCandidates = [] }
                return
            }
            let candidates = peer.endpoints.compactMap(PeerEndpoint.init)
            let requestedAt = peer.connectRequestedAt
            Punch.queue.async { [weak self] in
                guard let self else { return }
                if candidates != peerCandidates {
                    peerCandidates = candidates
                    Punch.logger.notice(
                        "phone candidates: \(Punch.describe(candidates), privacy: .public)"
                    )
                    // New addresses: don't wait out the idle interval, the
                    // phone may be dialling them right now.
                    nextPingAt = .distantPast
                }
                if let requestedAt,
                   Date().timeIntervalSince(requestedAt) < Self.urgentWindow {
                    if urgentUntil < Date() {
                        let age = Int(Date().timeIntervalSince(requestedAt))
                        let aiming = Punch.describe(peerCandidates)
                        Punch.logger.notice(
                            "phone is dialling (request \(age, privacy: .public)s old) — punching at \(aiming, privacy: .public)"
                        )
                    }
                    // From now, not from `requestedAt`: this poll can be up
                    // to `pollInterval` late, and anchoring to the request
                    // time silently spent that lateness out of the window we
                    // punch in. A request seen 8s old used to buy 12s of hard
                    // punching, against a phone that tries for far longer.
                    urgentUntil = Date().addingTimeInterval(Self.urgentWindow)
                    nextPingAt = .distantPast
                }
            }
        }
    ...
    private func receive(_ data: Data, from source: PeerEndpoint) {
        guard running, let socket, let key else { return }
        ...
        guard let packet = PunchPacket.decode(data, key: key) else {
            ...
            // means the phone's packets ARE arriving and the key is wrong.
            rejected += 1
            if rejected % 10 == 1 {
...
final class PunchClient {
    static let shared = PunchClient()
    ...
    /// A stream to the Mac, or a message fit to show the user.
    func open() async -> Result<PunchTransport, PairingError> {
        guard PairingStore.load() != nil, let key = try? PairingStore.channelKey() else {
            return .failure(.network("This iPhone isn't paired with a Mac yet."))
        }
        let peer: PeerEndpoint
        switch await path(key: key) {
        case let .success(found): peer = found
        ...
        return await withCheckedContinuation { cont in
            Punch.queue.async { [self] in
                ...
                let stream = PunchTransport(
                    socket: socket, peer: peer, session: session, key: key,
    ...
    func reachable() async -> Result<Void, PairingError> {
        guard PairingStore.load() != nil, let key = try? PairingStore.channelKey() else {
            return .failure(.network("This iPhone isn't paired with a Mac yet."))
        }
        return await path(key: key).map { _ in () }
    }
    ...
    private func path(key: SymmetricKey) async -> Result<PeerEndpoint, PairingError> {
                ...
                if socket == nil {
                    do {
                        ...
                        cont.resume(returning: .failure(
                            .network("Couldn't open a network connection on this iPhone.")
                        ))
                        return
                    }
                }
    ...
    private func startTimer(key: SymmetricKey) {
        timer?.cancel()
        let t = DispatchSource.makeTimerSource(queue: Punch.queue)
        t.schedule(deadline: .now(), repeating: Self.probeInterval)
        t.setEventHandler { [weak self] in self?.tick(key: key) }
        t.resume()
        timer = t
    }

    private func tick(key: SymmetricKey) {
    ...
    private func fetchPeer() {
        Task { [cloud] in
            ...
            let found = peer.endpoints.compactMap(PeerEndpoint.init)
            let age = Int(Date().timeIntervalSince(peer.heartbeatAt))
            Punch.queue.async { [weak self] in
                guard let self else { return }
                if found != candidates {
    ...
    private func receive(_ data: Data, from source: PeerEndpoint, key: SymmetricKey) {
        ...
        if stun.handled {
            ...
            // Mac can aim at it.
            if stun.changed, dial == .punching { announce() }
            return
        }
